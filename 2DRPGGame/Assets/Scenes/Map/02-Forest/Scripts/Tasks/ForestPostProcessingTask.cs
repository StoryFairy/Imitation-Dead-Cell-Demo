...
[CreateAssetMenu(menuName = "Map/Forest/Post-processing", fileName = "Forest Post Processing")]
public class ForestPostProcessingTask : DungeonGeneratorPostProcessingGrid2D,IMMEventListener<CorgiEngineEvent>
{
    ...
    private void SetSpawnPosition(DungeonGeneratorLevelGrid2D level)
    {
        var entranceRoomInstance =
            level.RoomInstances.FirstOrDefault(x => ((ForestRoom)x.Room).Type == ForestRoomType.Entrance);
    ...
    {
        var environmentLayer = LayerMask.NameToLayer(ForestGameManager.StaticEnvironmentLayer);
    ...
    {
        var entranceRoomInstance =
            level.RoomInstances.FirstOrDefault(x => ((ForestRoom)x.Room).Type == ForestRoomType.Shop);
        var roomTemplateInstance = entranceRoomInstance.RoomTemplateInstance;
    ...
    public void OnMMEvent(CorgiEngineEvent eventType)
    {
        ...
        if (eventType.EventType == CorgiEngineEventTypes.LevelStart && LevelManager.Instance.LevelName=="02-Forest")
        {
