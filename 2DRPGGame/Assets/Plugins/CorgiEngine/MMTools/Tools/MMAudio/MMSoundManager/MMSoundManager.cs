...
public class MMSoundManager : MMPersistenSingleton<MMSoundManager>,
    IMMEventListener<MMSoundManagerTrackEvent>,
    IMMEventListener<MMSoundManagerEvent>,
    IMMEventListener<MMSoundManagerSoundControlEvent>,
    IMMEventListener<MMSoundManagerSoundFadeEvent>,
    IMMEventListener<MMSoundManagerAllSoundsControlEvent>,
    IMMEventListener<MMSoundManagerTrackFadeEvent>
{
    ...
    )
    {
        ...
        if (audioSource == null)
        {
            ...
            if ((audioSource != null) && (!loop))
            {
                ...
                StartCoroutine(
                    _pool.AutoDisableAudioSource(audioClip.length / Mathf.Abs(pitch), audioSource, audioClip));
            }
        }
